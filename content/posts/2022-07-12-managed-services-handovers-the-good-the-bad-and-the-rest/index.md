...
> The remainder of these assumes the above **DOESN'T** exist
...
{{< collapsible title="**Be explicit about versions**" >}}
...
{{< collapsible title="**Mobile or Simulator setup** AND **Cross platform instructions**" >}}
...
- **Workarounds/hacks**
  ...
  - Personal client app registrations and secret, magic string to bypass auth, `appsettings.local.json` with no example or setup instructions on what **SHOULD** be in there for sensible local dev, even a local database snapshot that makes dev usable.  
  - Not having reasons makes it difficult to be confident during a change
- **Magic strings/tokens**
  - Similar to above, but these are more deliberate.
...
It's the ultimate fix for "works on my machine".  
