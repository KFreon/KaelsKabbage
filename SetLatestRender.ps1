...
if ($ext -eq '.webm') {
    Start-Process ffmpeg "-i $recentRender -y -vframes 1 -vf scale=300:-1 $destination" -Wait -RedirectStandardError ./latest-render-results
} else {
    Start-Process ffmpeg "-i $recentRender -y -vf scale=300:-1 $destination" -Wait -RedirectStandardError ./latest-render-results
}
